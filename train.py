...
SPECIAL_TOKENS = ["<bos>", "<eos>", "<speaker1>", "<speaker2>", "<pad>"] #, '<S>', '<ES>', '<R>', '<ER>', '<T>', '<ET>']
...
def get_data_loaders(args, tokenizer):
    ...
    personachat = get_dataset(tokenizer, args.dataset_path, min_candidates=args.min_candidates, max_candidates=args.num_candidates)
...
def train():
    ...
    parser.add_argument("--min_candidates", type=int, default=1, help="Number of candidates for training. Smaller numbers will will limit training size but reduce duplication")
    ...
    if args.local_rank in [-1, 0] and args.n_epochs > 0:
        os.rename(checkpoint_handler._saved[-1][1][-1], os.path.join(tb_logger.writer.logdir, WEIGHTS_NAME))  # TODO (huggingface): PR in ignite to have better access to saved file paths (cleaner)
        tb_logger.close()
